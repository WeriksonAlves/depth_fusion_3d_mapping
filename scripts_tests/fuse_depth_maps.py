...
def fuse_maps(rgb_dir, depth_real_dir, depth_mono_dir, T_path, output_dir, intrinsics, depth_scale=5000.0):
    ...
    for i, (rgb_path, real_path, mono_path) in enumerate(zip(rgb_files, real_files, mono_files)):
        ...
        # Criar point cloud monocular e aplicar transformação
        ...
        # Projetar a nuvem transformada para imagem de profundidade
        depth_projected = project_point_cloud_to_depth(pcd_mono, intrinsics, depth_scale)

        # Fusão: manter real, preencher com monocular onde real é 0
        real_np = np.asarray(depth_real).astype(np.uint16)
        fused = np.where(real_np == 0, depth_projected, real_np)
...
def main():
    ...
    fuse_maps(
        ...
        T_path=base / "results/T_d_to_m.npy",
        output_dir=base / "results/depth_fused",
