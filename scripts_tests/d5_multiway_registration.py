...
class MultiwayReconstructorOffline:
    ...
    def __init__(
        ...
        output_path: Path,
        output_pcd: Path = None,
        voxel_size: float = 0.02
    ) -> None:
        self.rgb_dir = rgb_dir
        self.depth_npy_dir = depth_npy_dir
        ...
        if not self.rgb_dir.exists():
            raise FileNotFoundError(
                f"[ERROR] Dataset not found: {self.rgb_dir}"
            )
        if not self.depth_npy_dir.exists():
            raise FileNotFoundError(
                f"[ERROR] Depth directory not found: {self.depth_npy_dir}"
            )
        if not self.intrinsics_path.exists():
            raise FileNotFoundError(
                f"[ERROR] Intrinsics JSON not found: {self.intrinsics_path}"
            )
    ...
    def run(self) -> None:
        ...
        point_clouds = RGBDLoader.load_point_clouds(
            self.rgb_dir, self.depth_npy_dir, intrinsic, self.voxel_size
        )
        ...
        o3d.io.write_point_cloud("results/lab_scene_f/d5/final_reconstruction.ply", merged)
